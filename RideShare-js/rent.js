...
function createVehicleCard(vehicle) {
    ...
    // Get image URL with proper fallback to placeholder.com
    const imageUrl = (vehicle.images && vehicle.images[0]) || vehicle.image || 'https://via.placeholder.com/400x200/1e3a5f/ffffff?text=No+Image';
    
    card.innerHTML = `
        <div class="vehicle-image-container">
            <img src="${imageUrl}" 
                 alt="${vehicle.name}" class="vehicle-image"
                 onerror="this.src='https://via.placeholder.com/400x200/1e3a5f/ffffff?text=No+Image'">
            <div class="vehicle-badge ${vehicle.available ? '' : 'rented'}">
                ${vehicle.available ? 'Available' : 'Rented'}
            </div>
            <div class="vehicle-favorite ${isFavorite ? 'active' : ''}" data-vehicle-id="${vehicle.id}">
